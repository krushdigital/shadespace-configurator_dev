...
export function ShadeConfigurator() {
  ...
  const handleSendSaveProgressEmail = async (
    firstName: string,
    lastName: string,
    email: string,
    quoteName: string,
    ...
    try {
      ...
      if (svgElement) {
        try {
          ...
          console.error('Error processing canvas image for save progress email:', error);
        }
      }
      ...
      const response = await fetch(
        ...
        {
          method: "POST",
          body: JSON.stringify({ pdf: null, ...orderData, email, firstName, lastName, quoteUrl }),
        }
      );
      ...
      console.error("Save progress email send failed:", error);
      return false;
    }
  };
  ...
  return (
    <>
      ...
      <UnifiedSaveModal
        ...
        onSendSaveProgressEmail={handleSendSaveProgressEmail}
      />
    </>
  );
}