...
export const ConfigurationChecklist = forwardRef<ConfigurationChecklistRef, ConfigurationChecklistProps>((
  ...
  useImperativeHandle(ref, () => ({
    ...
    expandHeights: () => {
      // Heights are no longer expandable on review page - navigation to dimensions step is handled elsewhere
      // This is kept for interface compatibility but does nothing
    },
    getHeightSectionElement: () => null,
  }));
        ...
        {shouldShowDiagonalInputSection && (
          <div
            ...
            className={`rounded-lg transition-all duration-300 ${
              isHighlighted
                ? 'border-2 border-red-500 ring-4 ring-red-300 shadow-xl pulse-error bg-red-50'
                : allDiagonalsEntered
          ...
          >
            <div
              className={`flex items-start gap-3 p-3 cursor-pointer transition-colors ${
                !diagonalsExpanded ? (allDiagonalsEntered ? 'hover:bg-blue-50/50' : 'hover:bg-amber-100/50') : ''
              }`}
              ...
              <div className="flex-1">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-1.5">
                    <span className={`text-sm font-medium ${allDiagonalsEntered ? 'text-emerald-900' : 'text-amber-900'}`}>
                    ...
                    <div onClick={(e) => e.stopPropagation()}>
                      <Tooltip content="Diagonal measurements ensure manufacturing accuracy and help our team create your exact shade shape with precision.">
                        <span className={`${allDiagonalsEntered ? 'text-emerald-700 hover:text-emerald-900' : 'text-amber-700 hover:text-amber-900'} inline-flex items-center justify-center`} role="button" tabIndex={0}>
                  ...
                  <button
                    ...
                    className={`${allDiagonalsEntered ? 'text-emerald-700 hover:text-emerald-900' : 'text-amber-800 hover:text-amber-900'} font-medium text-sm bg-transparent border-0 p-0 cursor-pointer whitespace-nowrap`}
                  >
            ...
            {diagonalsExpanded && (
              <div className={`px-3 pb-3 pt-0 border-t mt-2 ${allDiagonalsEntered ? 'border-blue-200' : 'border-amber-300'}`}>
                <div className={`rounded-lg p-3 mb-3 mt-3 transition-colors duration-300 ${
                  isHighlighted ? 'bg-red-100 border-2 border-red-400' : allDiagonalsEntered ? 'bg-emerald-50 border border-emerald-200' : 'bg-white border border-amber-200'
                }`}>
                  <p className={`text-xs font-semibold ${
                    isHighlighted ? 'text-red-900' : allDiagonalsEntered ? 'text-emerald-800' : 'text-amber-800'
                  }`}>
                    {isHighlighted ? '⚠️ Please enter all diagonal measurements below to proceed:' : allDiagonalsEntered ? '✓ All diagonal measurements have been entered.' : 'Measure the straight-line distance between non-adjacent corners. Enter all measurements below.'}
                  </p>
                </div>
        ...
        {showHeightRequired && (
          <div className={`flex items-start gap-3 p-3 rounded-lg transition-all duration-300 ${
            ...
            <div className="flex-1">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-1.5">
                  <span className={`text-sm font-medium ${allHeightsProvided ? 'text-emerald-900' : 'text-amber-900'}`}>
                    Height information {allHeightsProvided ? 'complete' : 'required'}
                  </span>
                  <Tooltip content={`Shade sails with ${config.corners} corners require height measurements for each fixing point. This ensures proper tension, water runoff, and structural integrity for complex installations.`}>
                    <span className={`${allHeightsProvided ? 'text-emerald-700 hover:text-emerald-900' : 'text-amber-700 hover:text-amber-900'} inline-flex items-center justify-center`} role="button" tabIndex={0}>
                      <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                        <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-8-3a1 1 0 00-.867.5 1 1 0 11-1.731-1A3 3 0 0113 8a3.001 3.001 0 01-2 2.83V11a1 1 0 11-2 0v-1a1 1 0 011-1 1 1 0 100-2zm0 8a1 1 0 100-2 1 1 0 000 2z" clipRule="evenodd" />
                      </svg>
                    </span>
                ...
                {onNavigateToHeights && !allHeightsProvided && (
                  <button
                    onClick={onNavigateToHeights}
                    className="text-sm font-medium text-amber-800 hover:text-amber-900 px-2 py-1 rounded hover:bg-amber-100 transition-colors whitespace-nowrap"
                  >
                    Enter Heights →
                  </button>
                )}
