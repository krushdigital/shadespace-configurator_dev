...
function DimensionOverlay({
  ...
  const { labels, lines } = useMemo(() => {
    ...
    for (const [key, value] of Object.entries(config.measurements)) {
      ...
      const path = buildMeasurementPath(key, measurementOption, fixingPointPositions, sailAttachPoints);
  ...
  }, [config.measurements, config.fixingHeights, config.corners, (config as any).unit, config.measurementOption, fixingPointPositions, sailAttachPoints, centroid]);
