  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      if (accessToken) {
        ...
        return new Response(
          JSON.stringify({ error: 'Missing required parameter: token, id, or reference' }),
          {
            status: 400,
            headers: { ...corsHeaders, 'Content-Type': 'application/json' }
          }
    ...
    if (req.method === 'PATCH') {
      const { id, status, accessToken } = await req.json();
      ...
      if (accessToken) {
        updateQuery = updateQuery.eq('access_token', accessToken);
      }
