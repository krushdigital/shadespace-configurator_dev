...
import { Wrench, Eye, EyeOff, Star, AlertTriangle, ImageOff, Copy, X, Search, GitMerge, GripVertical } from 'lucide-react';
...
function SortableCategoryItem({ category }: { category: CategoryRow }) {
  ...
  return (
    <div ref={setNodeRef} style={style} className="inline-flex items-center gap-1 rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-700 select-none">
      <button type="button" {...attributes} {...listeners} className="cursor-grab active:cursor-grabbing p-0.5 -ml-1 text-slate-400 hover:text-slate-600">
        <GripVertical className="w-3 h-3" />
      </button>
      {category.label}
      <span className="text-slate-400">#{category.display_order}</span>
    </div>
  );
}
...
export const HardwareCatalogManager: React.FC = () => {
  ...
  const handleCategoryDragEnd = async (event: DragEndEvent) => {
    ...
    const updates = reordered.map(c =>
      supabase.from('hardware_categories').update({ display_order: c.display_order }).eq('id', c.id)
    );
    ...
    if (failed?.error) {
      showToast(`Failed to save order: ${failed.error.message}`, 'error');
      load();
    } else {
    ...
    <Card className="p-5">
      ...
      <div className="mt-6 border-t border-slate-200 pt-4">
        <div className="flex items-center justify-between mb-2">
          ...
          {savingCategoryOrder && <span className="text-xs text-slate-500">Saving...</span>}
        </div>
        <p className="text-xs text-slate-500 mb-2">Drag to reorder. Order determines display in the hardware selection modal.</p>
        <DndContext sensors={categorySensors} collisionDetection={closestCenter} onDragEnd={handleCategoryDragEnd}>
          <SortableContext items={categories.map(c => c.id)} strategy={horizontalListSortingStrategy}>
            <div className="flex flex-wrap gap-2 mb-3">
              {categories.map(c => (
                <SortableCategoryItem key={c.id} category={c} />
              ))}
            </div>
          </SortableContext>
        </DndContext>
